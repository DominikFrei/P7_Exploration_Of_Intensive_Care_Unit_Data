...
data_death <- data %>% select(censor_flg, day_28_flg, hosp_exp_flg, icu_exp_flg, aline_flg)
#correlation matrix
...
Produce boxplots to see the general distribution of the data (outliers are not shown)
```{r fig.height=5}
boxplot(data_meas[,1:6], outline= FALSE)
```
...
```{r fig.height=5}
boxplot(data_meas[,7:12], outline= FALSE)
```
All are more or less symmetric.
```{r fig.height=5}
boxplot(data_meas[,13:18], outline= FALSE)
```
