...
which of the variables are correlated with death shortly after icu admission.
...
We will now go on to remove always the variable with the highest p-value:
```{r}
data_28d <- data_28d %>% select(!platelet_first)
mlreg_4 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_4)
```
```{r}
data_28d <- data_28d %>% select(!pco2_first)
mlreg_5 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_5)
```
```{r}
data_28d <- data_28d %>% select(!service_unit)
mlreg_6 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_6)
```
```{r}
data_28d <- data_28d %>% select(!copd_flg)
mlreg_7 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_7)
```
```{r}
data_28d <- data_28d %>% select(!spo2_1st )
mlreg_8 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_8)
```
```{r}
data_28d <- data_28d %>% select(!cad_flg)
mlreg_9 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_9)
```
```{r}
data_28d <- data_28d %>% select(!iv_day_1)
mlreg_10 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_10)
```
```{r}
data_28d <- data_28d %>% select(!tco2_first)
mlreg_11 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_11)
```
```{r}
data_28d <- data_28d %>% select(!po2_first)
mlreg_12 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_12)
```
```{r}
data_28d <- data_28d %>% select(!potassium_first)
mlreg_13 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_13)
```
```{r}
data_28d <- data_28d %>% select(!abg_count)
mlreg_14 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_14)
```
```{r}
data_28d <- data_28d %>% select(!aline_flg)
mlreg_14 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_14)
```
```{r}
data_28d <- data_28d %>% select(!map_1st)
mlreg_15 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_15)
```
```{r}
data_28d <- data_28d %>% select(!chf_flg)
mlreg_16 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_16)
```
```{r}
data_28d <- data_28d %>% select(!liver_flg)
mlreg_17 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_17)
```
```{r}
data_28d <- data_28d %>% select(!wbc_first)
mlreg_18 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_18)
```
```{r}
data_28d <- data_28d %>% select(!renal_flg)
mlreg_19 <- glm(day_28_flg ~ ., family = "binomial", data = data_28d)
summary(mlreg_19)
```

```{r}
a <- coef(summary(mlreg_19)) %>% data.frame() %>% rename(., pval = "Pr...z..") %>% .[-1,]
colnames(a)
to_exc <- a[which(a$pval == max(a$pval)),]
```


Write backwards selection function:
```{r}
back_sel_logreg <- function(dataset, outcome){
