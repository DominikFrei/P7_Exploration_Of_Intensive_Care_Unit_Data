...
Since our goal is prediction we will do a three way split, train:test (6:4)
```{r}
#set seed
set.seed(1)
...
The model is not that good in predicting the los (adj R2 = 0.4764).
...
We will now try to manually select the variables to include.
...
the amount of days (halve days) a person spent in the ICU? Maybe the "higher accuracy"
of the result is just added noise that doesnt really mean anything and is not at all 
relevant for the organization.

We could investigate, if we should just round the outcome to halve or whole days, 
which would probably be what is relevant for the problem. ????????


Lets find out what time the results should be rounded to:
```{r}
...
The created weekend variable doesnt seem to contain any information.
...
the prediction should be adapted, otherwise it doesnt add anything to the mode, except
for more realistic predicted discharge times (which isnt relevant here).

We will use lasso to find the best model:
```{r}
set.seed(1)
...
```
This result is not great with an adj R squared of 0.21.
...
```
Many of these displayed relationshipd dont seem to be represented very well by a linear model.
...
in the model. This is also in line with the poor model fit (adjusted R2).
...
data_los_icu_train_droped %>% select_if(is.numeric) %>%         
        ...
        theme_classic()
```
Most of these look ok. Age should probably also have been logarithmized.
...
bind_cols(data_los_icu_test_prep, 
        ...
        theme_classic()
```
We never get lower then 1 day. Mostly the error is around 1/2 of the LOS
of the patient.
...
I think th epoor model performance is probably due to the available data not beeing
relevant enough to predict the outcome variable. There seems to be a lot of unexplained variance
in the outcome. The data that was used was collected from a database but with another
...
To predict LOS in ICU is probably also quite a hard problem. Although there are 
sources that indicate, this can be done more successfully ().
Maybe LOS in hospital would have been a more reasonable choice.
