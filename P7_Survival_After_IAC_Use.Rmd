...
It is unclear to me if these measurements where wrong or not. We could
possibly replace them with NAs and impute them later. I think it also depends on the weight
these outliers would get in a model
...
```{r}
data <- data %>% mutate(service_unit = recode(service_unit, "FICU" = "MICU"))
```
...
more likely be due to accidents.  
...
Lets look at the correlations between all the variables:
```{r}
...
```{r}
table(data$gender_num, useNA = "ifany")
```
